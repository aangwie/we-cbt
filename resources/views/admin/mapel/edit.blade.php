@extends('layouts.app')
@section('title', 'Edit Mata Pelajaran - WeTest')
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
            <form action="{{ route('admin.mapel.update', $mapel) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="kode_mapel" value="{{ old('kode_mapel', $mapel->kode_mapel) }}" required placeholder="Contoh: MAT" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition outline-none uppercase">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1.5">Nama Mata Pelajaran</label>
                    <input type="text" name="nama_mapel" value="{{ old('nama_mapel', $mapel->nama_mapel) }}" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition outline-none">
                </div>
