@extends('layouts.app')
@section('title', 'Edit Soal - WeTest')
...
@section('content')
    <div class="max-w-3xl">
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
            <form action="{{ route('admin.soal.update', $soal) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf @method('PUT')
                <div>
                    ...
                    <select name="ujian_id" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition outline-none">
                        @foreach($ujians as $ujian)
                            <option value="{{ $ujian->id }}" {{ old('ujian_id', $soal->ujian_id) == $ujian->id ? 'selected' : '' }}>{{ $ujian->judul }} ({{ $ujian->guru->name }})</option>
                        @endforeach
                    </select>
                </div>
                    ...
                    <div>
                        ...
                        <select name="mapel_id" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition outline-none">
                            ...
                            @foreach($mapels as $mapel)
                                <option value="{{ $mapel->id }}" {{ old('mapel_id', $soal->mapel_id) == $mapel->id ? 'selected' : '' }}>{{ $mapel->nama_mapel }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="kelas" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition outline-none">
                            ...
                            @foreach($kelasList as $k)
                                <option value="{{ $k->nama_kelas }}" {{ old('kelas', $soal->kelas) == $k->nama_kelas ? 'selected' : '' }}>{{ $k->nama_kelas }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="teks_soal" rows="3" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition outline-none resize-none">{{ old('teks_soal', $soal->teks_soal) }}</textarea>
                </div>
                <div>
                    ...
                    @if($soal->gambar_soal)
                        <div class="mb-2"><img src="{{ asset('storage/' . $soal->gambar_soal) }}" alt="Gambar soal" class="h-24 rounded-lg border"></div>
                    @endif
                ...
                <div class="border-t border-slate-100 pt-6 space-y-4">
                    ...
                    <div class="bg-slate-50 rounded-xl p-4 space-y-3">
                        <div class="flex items-center gap-2">
                            ...
                            <input type="text" name="pilihan_{{ $opt }}" value="{{ old('pilihan_' . $opt, $soal->{'pilihan_' . $opt}) }}" required class="flex-1 px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition outline-none">
                        </div>
                        @if($soal->{'gambar_' . $opt})
                            <div><img src="{{ asset('storage/' . $soal->{'gambar_' . $opt}) }}" alt="Gambar {{ $opt }}" class="h-16 rounded-lg border"></div>
                        @endif
                ...
                <div>
                    ...
                    <select name="jawaban_benar" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition outline-none">
                        @foreach(['a', 'b', 'c', 'd', 'e'] as $opt)
                            <option value="{{ $opt }}" {{ old('jawaban_benar', $soal->jawaban_benar) === $opt ? 'selected' : '' }}>{{ strtoupper($opt) }}</option>
                        @endforeach
                    </select>
                </div>

                <div class="flex items-center gap-3 pt-2">
                    <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-sm font-semibold rounded-xl hover:from-blue-700 hover:to-indigo-700 transition shadow-lg shadow-blue-500/25">Update Soal</button>
                    <a href="{{ route('admin.soal.index') }}" class="px-6 py-2.5 bg-slate-100 text-slate-600 text-sm font-semibold rounded-xl hover:bg-slate-200 transition">Batal</a>
                </div>
            </form>
        </div>
    </div>
