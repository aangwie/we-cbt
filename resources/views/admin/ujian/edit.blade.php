...
@section('content')
    <div class="max-w-2xl">
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
            <form action="{{ route('admin.ujian.update', $ujian) }}" method="POST" class="space-y-5" x-data="{
                kelas: '{{ old('kelas', $ujian->kelas ?? '') }}',
                selectedMapel: '{{ old('mapel_id', $ujian->mapel_id) }}',
                selectedPaket: '{{ old('paket_soal_id', $ujian->paket_soal_id) }}',
                ...
                <div>
                    ...
                    <select name="mapel_id" x-model="selectedMapel" @change="selectedPaket = ''" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition outline-none">
