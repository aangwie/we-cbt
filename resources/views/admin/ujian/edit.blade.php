        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
            <form action="{{ route('admin.ujian.update', $ujian) }}" method="POST" class="space-y-5" x-data="{ kelas: '{{ old('kelas', $ujian->kelas ?? '') }}' }">
                ...
                <div>
                    ...
                    <input type="text" name="judul" value="{{ old('judul', $ujian->judul) }}" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition outline-none">
                </div>
                <div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        ...
                        <label class="relative cursor-pointer">
                            <input type="radio" name="kelas" value="{{ $k }}" x-model="kelas" class="sr-only peer" {{ old('kelas', $ujian->kelas) === $k ? 'checked' : '' }}>
